...
// ── Custom event so Profile page can tell Navbar "photo updated" ──────────────
// Profile.jsx fires: window.dispatchEvent(new CustomEvent('kamai:photo-updated', { detail: url }))
export const PHOTO_UPDATED_EVENT = 'kamai:photo-updated'
  ...
  return (
    <div className="relative group flex items-center h-full">
      <button type="button"
        ...
        About App
        <svg className="w-3.5 h-3.5 transition-transform duration-200 group-hover:rotate-180"
          fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" />
        </svg>
      </button>
      <div className="absolute top-full left-0 w-full h-4 opacity-0" aria-hidden="true" />
...
function Avatar({ photoSrc, initials, size = 'sm' }) {
  const dim = size === 'sm' ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm'
  const ring = size === 'sm' ? 'ring-2 ring-teal-400' : 'ring-2 ring-teal-400'
  return photoSrc ? (
    <img src={photoSrc} alt="avatar"
      className={`${dim} rounded-full object-cover ${ring}`} />
  ) : (
    <div className={`${dim} rounded-full bg-teal-500 flex items-center justify-center
      text-white font-bold ${ring}`}>
      {initials}
    </div>
  )
}

function UserMenu({ user, photoSrc }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(o => !o)}
        ...
        <Avatar photoSrc={photoSrc} initials={initials} size="sm" />
...
export default function Navbar({ activePage }) {
  ...
  // Fetch MySQL photo_url once per login, fall back to Google photoURL
  const fetchPhoto = async (firebaseUser) => {
    if (!firebaseUser) { setPhotoSrc(null); return }
    try {
      const data = await getProfile()
      // MySQL upload takes priority; fall back to Google/Firebase photo
      setPhotoSrc(data.photo_url || firebaseUser.photoURL || null)
    } catch {
      setPhotoSrc(firebaseUser.photoURL || null)
    }
  }
  ...
  // Listen for profile saves so avatar updates immediately without a page reload
  useEffect(() => {
    const handler = (e) => setPhotoSrc(e.detail || null)
    ...
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between h-14 sm:h-16">
        ...
        <a href="/" className="flex items-center gap-2 flex-shrink-0">
          <img src="/assets/logo.png" alt="KamAI logo" className="h-9 w-auto"
        ...
        <div className="hidden md:flex items-center gap-4">
          {user ? (
            <UserMenu user={user} photoSrc={photoSrc} />
          ) : (
            <>
              <a href="/signin" className="text-sm font-medium text-gray-700 hover:text-teal-500 transition-colors">Sign In</a>
              <a href="/signup" className="btn-shimmer text-white text-sm font-semibold px-5 py-2 rounded-lg">Sign Up</a>
            </>
          )}
        </div>
        ...
        <div className="md:hidden fixed inset-0 top-14 bg-white z-40 px-4 py-6 flex flex-col gap-1 animate-fadeIn overflow-y-auto">
          ...
          {user && (
            <div className="flex items-center gap-3 px-3 py-4 mb-2 bg-teal-50 rounded-xl">
              <Avatar photoSrc={photoSrc} initials={initials} size="lg" />
              <div>
          ...
          {user ? (
            ...
            <div className="flex gap-3 mt-6">
              <a href="/signin" className="flex-1 text-center py-3 text-sm font-semibold text-gray-700 border border-gray-200 rounded-xl hover:border-teal-400 hover:text-teal-600 transition-colors">Sign In</a>
              <a href="/signup" className="flex-1 text-center py-3 text-sm font-semibold text-white rounded-xl btn-shimmer">Sign Up</a>
            </div>
          )}
        </div>
      )}
